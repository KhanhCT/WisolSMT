...
namespace SMTMornitor.Converter
{
    public class TextToColorConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            else if ((para == "jig-shortage" && Status == OrderShortageReason.JIGShortage))
            {
                return new SolidColorBrush(Colors.Orange);
            }
            else if ((para == "pcb-shortage" && Status == OrderShortageReason.PCBShortage))
            {
                return new SolidColorBrush(Colors.Orange);
            }
            else if ((para == "magazin-shortage" && Status == OrderShortageReason.MGZShortage))
            {
                return new SolidColorBrush(Colors.Orange);
            }
            else if ((para == "plasma-waiting" && Status == OrderShortageReason.PLMWaiting))
            {
                return new SolidColorBrush(Colors.Orange);
            }
            else
