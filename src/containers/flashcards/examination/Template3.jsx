...
export default class Template3 extends Component {
    ...
    processData = () => {
        ...
        if (questionData) {
            ...
            return { question, arrCharacter, userAnswer };
        }
    };
    ...
    componentWillUnmount = () => {
        const { question, arrCharacter, userAnswer } = this.state;
        if (!this.props.preview)
            this.props.updateExaminationData({
                templateNumber: 3,
                templateData: { question, arrCharacter, userAnswer },
            });
    };
    ...
    render() {
        const { question, arrCharacter } = this.state;
        ...
        let arrayCharacterAnswer = question["name"][lang].split("");
        arrayCharacterAnswer = arrayCharacterAnswer.map((character, index) => {
            return {
                content: character,
                id: `item-${index}`,
            };
                ...
                <DragDropContext onDragEnd={this.onDragEnd}>
                    <Droppable droppableId="droppable" direction="horizontal">
                        {(provided, snapshot) => (
                            ...
                            >
                                {arrayCharacterAnswer.map((item, index) => (
                                    <Draggable
                                        key={index}
                                        draggableId={item.id}
                                        index={index}
        ...
        return (
            <Container className="mw-960 sm-pl-1 sm-pr-1">
                ...
                <div>
                    {preview ? <strong>Your Answer</strong> : ""}
                    <DragDropContext onDragEnd={this.onDragEnd}>
                        <Droppable
                        ...
                        >
                            {(provided, snapshot) => (
                                <div
                                    className={
                                        !preview
                                            ? "character-questions-border"
                                            : "character-questions-border bg-blue"
                                    }
                                    ref={provided.innerRef}
                                    style={getListStyle(
