...
export class ProductionPlan extends Component {
	...
	deletePlan = (row, column) => {
		let workingDate = moment(row.WorkingDate, "DD/MM/YYYY").format(
			"YYYY-MM-DD"
		);
		callApi(
			`productionplan/${workingDate}/${row.FactoryID}/${row.LineID}/${
				row.ShiftID
			}/${row.WorkPlaceID}/${row.ProductID}`,
			"DELETE",
			{},
			{}
		)
	...
	editPlan = (row, column) => {
		callApi(
			...
			{},
			{
				WorkingDate: moment(row.WorkingDate, "DD/MM/YYYY").format(
					"YYYY-MM-DD"
				),
	...
	productEditor = props => {
		return (
			<SelectProduct
				product={
					this.state.listProductionPlan[props.rowIndex]["Product"]
				}
